...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        for i in range(5):
            dictionary[vowels[i]] = vowels_permutation[i]
            dictionary[vowels[i].upper()] = vowels_permutation[i].upper()
            
        for i in consonants:
            dictionary[i] = i
            dictionary[i.upper()] = i.upper()
        ...
        for perm in vowel_permutations:
            ...
            for letter in self.message_text:
                if(letter.isalpha() and letter != ' '):
                    decrypted += current_dict[letter]
                else:
                    decrypted += letter
            
            decrypted.strip("!@#$%^&*()-_+={}[]|\:;'<>?,./\"")
            split_decrypted = decrypted.split(' ')
            current_valid_words = 0
            
            for j in split_decrypted:
